		...
		<section className={"w-full flex flex-col gap-8"}>
			<div className={"flex flex-col gap-4"}>
				...
				<div className={"w-full flex justify-center gap-32"}>
					<DetailBlock
						color={
							forecastDetails?.transaction_type &&
							["receive", "borrow", "repay_received"].includes(forecastDetails?.transaction_type)
								? "success"
								: "danger"
						}
						icon={SYS_ICONS.DOLLAR.LG}
						label={"Transaction Type"}
						value={forecastDetails?.transaction_type.replaceAll("_", " ") || "-"}
					/>
					<DetailBlock
						color={"secondary"}
						icon={SYS_ICONS.DOLLAR.LG}
