...
export default function AddTransaction({ orientation = "horizontal", onSuccess }: AddTransactionProps) {
	...
	/* FEATURE: HANDLE PARSE QUICK INPUT BY OPEN AI

	const [quickInputText, setQuickInputText] = useState<string>("");

	const {
		data: quickInputResult,
		loading: loadingQuickInput,
		error: errorQuickInput,
		fetch: fetchQuickInput,
	} = useFetch<IAPIResponse>("/transactions/openai", {
		method: "POST",
		body: {
			message: quickInputText,
		},
		skip: true,
	});

	useEffect(() => {
		if (quickInputResult && quickInputResult.results) {
			setNewTransaction({
				...newTransaction,
				transaction_date: new Date(quickInputResult.results.transaction_date).toISOString(),
				transaction_amount: quickInputResult.results.transaction_amount,
				transaction_type: quickInputResult.results.transaction_type,
				description: quickInputResult.results.description,
				direction: ["receive", "repay_received", "borrow"].includes(quickInputResult.results.transaction_type)
					? "in"
					: "out",
			});
			...
			addToast({
				title: "Success",
				description: quickInputResult.message,
				color: "success",
			});
		} else {
			console.log("quickInputResult", quickInputResult);
		}

		if (errorQuickInput) {
			const parseError = JSON.parse(errorQuickInput);
	...
	}, [quickInputResult, errorQuickInput]); */

	useEffect(() => {
		if (createTransactionError) {
			setValidateErrors(JSON.parse(createTransactionError).errors);
		}
		...
		<section className={"flex flex-col gap-4"}>
			{/* <div className={"flex items-end gap-2"}>
				<Input
					...
					placeholder={"Input transaction by AI"}
					value={quickInputText}
					variant={"bordered"}
					onValueChange={(e) => setQuickInputText(e)}
				/>
				<Button
					color={"primary"}
					isDisabled={!quickInputText}
					isLoading={loadingQuickInput}
					onPress={fetchQuickInput}
				>
					Parse
				</Button>
			</div>
			...
			<div className={clsx("w-full flex justify-center gap-2")}>
				{loadingCards ? (
					...
					<CustomForm
						resetButtonIcon
						className={clsx("w-full flex h-max gap-4", {
						...
						<div className={"w-full flex flex-col gap-4"}>
							...
							<div className={"flex flex-col gap-2"}>
								<Input
									isRequired
									endContent={"VND"}
									errorMessage={getFieldError(validateErrors, "transaction_amount")?.message}
									isInvalid={!!getFieldError(validateErrors, "transaction_amount")}
									label={"Amount"}
									labelPlacement={"outside"}
								...
								<div className={"flex items-center gap-1"}>
									{makeSuggestAmount(newTransaction.transaction_amount).map((val, index) => (
										<Chip
											key={index}
											classNames={{
