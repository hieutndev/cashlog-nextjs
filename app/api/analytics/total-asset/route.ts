...
import { getTotalAssetFluctuation } from "../../_services/analytics-services";
...
export const GET = async (request: Request) => {
    try {
        ...
        const results = await getTotalAssetFluctuation(userId, timePeriod, parsedSpecificTime);

        return Response.json({
            status: "success",
            message: "Total asset fluctuation data retrieved successfully",
            results,
        }, {
            status: 200
        });
