...
export async function getLandingDataStats() {
  ...
  const recurrings = await dbQuery<any[]>(QUERY_STRING.GET_TOTAL_RECURRINGS_COUNT);
  ...
  return {
    ...
    total_recurrings: recurrings && recurrings[0] ? Number(recurrings[0].total) : 0,
  };
}
